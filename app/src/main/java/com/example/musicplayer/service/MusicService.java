...
public class MusicService extends Service implements MediaPlayer.OnErrorListener, MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener, OnAudioFocusChangeListener {
    ...
    public static int playMode = 2;//1.Repeat Current Song 2.Repeat List 0.shuffle
    ...
    private void initPlayer() {
        ...
        mPlayer.setOnErrorListener(this);//Source Error
        mPlayer.setOnPreparedListener(this);//Source ready
        mPlayer.setOnCompletionListener(this);//Finish Play
    }
    ...
    public void onPrepared(MediaPlayer mediaPlayer) {
        ...
        mPlayer.start();//Start to Play music
    ...
    private void sentPreparedMessageToMain() {
        ...
        mMessage.arg1 = mPosition-1;
        mMessage.obj = mPlayer.isPlaying();
        try {
            //Send current playing location
            mMessenger.send(mMessage);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
            ...
            public void run() {
                try {
                    if (mPlayer.isPlaying()) {
                        //1.send the total time of the song to the activity when it is ready
        ...
        public void onReceive(Context context, Intent intent) {
            switch (intent.getAction()) {
                ...
                case Constants.ACTION_NEXT:
                    ...
                    if (playMode % 3 == 1) {//1.Repeat Current
                        play(mPosition);
                    } else if (playMode % 3 == 2) {//2.Play List
                        mPosition++;
                    ...
                    } else if (playMode % 3 == 0) {// 0.Shuffle
                        play(getRandom());
                    }
                    break;
                case Constants.ACTION_PRV:
                    ...
                    //Previous Song
                    prv_position = mPosition;
                    if (playMode % 3 == 1) {//1.Repeat Current Song
                        play(mPosition);
                    } else if (playMode % 3 == 2) {//2.Play List
                        mPosition--;
                        if (mPosition < 0) {
    ...
    /**
     * ---------------Dealing with Media File---------------
     **/
    @Override
    public void onAudioFocusChange(int focusChange) {
        switch (focusChange) {
            case AudioManager.AUDIOFOCUS_GAIN://Get Media File
